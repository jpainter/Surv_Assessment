...
d = y %>% 
    ...
    rename( categoryCombos.name = name ) %>%
    mutate(
    ...
    select( -categoryCombo, -dataElement, -dataSet ) 
...
datatable( d, caption = "Data Elements", filter = 'top', editable = TRUE,
...
count( d, dataSet.name, category.name, categoryCombos.name, timelyDays, periodType )  %>% kable
