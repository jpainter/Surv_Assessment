...
```{r packages, echo = TRUE, results='hold', message=FALSE, cache=FALSE, warning=FALSE}
...
```{r dataset_ous , results = 'asis' }
 ...
 for ( i in 1:nrow( mal.datasets ) ){
   print( dataset.ous.n( mal.datasets[i, 'Name'] ) )
 } 
...
## Locations of Org Units with number of confirmed malaria cases 
...
d = left_join( clinics , 
 ...
 # NB: add test to see if coords within country; if not, try reversing
    ...
    pal = colorNumeric('OrRd', data$value.scale )
    binpal <- colorBin('OrRd', data$value , 5, pretty = TRUE)
 ...
 leaflet( d ) %>% 
        ...
        addCircleMarkers( 
            ...
            fillColor = ~factpal(level) , fillOpacity = 1
        ) %>%
        addScaleBar() %>%
...
```{r trend_plots, fig.width=10, fig.height=12 }
