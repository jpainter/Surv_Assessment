...
# list of required packages
    package.list = c("rlist", "listviewer", "DT", "knitr", "kableExtra", "xts", "leaflet",
                     "geojsonR", "RColorBrewer", "DT", "dygraphs", "httr", "jsonlite", "XML", "assertthat", "lubridate", "scales", "RcppRoll", "zoo", "gridExtra", "futile.logger", "Hmisc", "stringi", "rlang" , "tidyselect" , "tidyverse", "readxl" )
...
```{r ous_by_level}
 ...
 t = table( year( ymd_hms( ous$openingDate )) ) %>% addmargins(.) # but some are recent
...
admins = md$organisationUnits %>% 
    ...
    mutate(
        ...
        char_js = map( str_js , ~FROM_GeoJson(url_file_string = .x ) )
    )
...
```{r}
    ...
    binpal <- colorBin( c('#e41a1c','#377eb8','#4daf4a','#984ea3','#ff7f00'), 
                        clinics$Year , 2014:2018 )

    factpal <- colorFactor( c('#e41a1c','#377eb8','#4daf4a','#984ea3','#ff7f00'),  clinics$Year, reverse = TRUE )

 leaflet( clinics ) %>% 
        addTiles(  urlTemplate = 
                       "http://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
                   )  %>%
        # addGeoJSON(geojson = admins$coordinates,  color = "red",fill = "red") %>%
        addCircles( 
            ~long, ~lat, 
            radius =  ~level ,
            weight =  5 ,
            ...
            fillColor = ~factpal(Year) , fillOpacity = .75
        ) %>%
        addScaleBar() %>%
        # addLegend(pal = factpal, values = ~level , opacity = 1 ) %>%
        addLegend(pal = factpal, values = ~Year, opacity = 1 )
...
# Number of facilities are reporting each data element? 
...
```{r}
  ...
  if ( !file.exists( reported_data_file) ){
        ...
        d.reported = api_last12months_national_data( de.include = de.mal )
...
```{r select_data_elements}
    ...
    l = map( seq_along( searches ) ,
        ...
        mutate(  Include = NA ) %>%
        select( Include, id, name , dataSets, Group , categoryCombo) %>%
        rename( dataElement.id = id, dataElement = name ) %>% 
        arrange( Group, dataSets,  dataElement )
    )
