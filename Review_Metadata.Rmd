...
# list of required packages
    package.list = c("rlist", "listviewer", "DT", "knitr", "kableExtra", "xts", "leaflet", "geojsonR", "rmapshaper" , "RColorBrewer", "DT", "dygraphs", "httr", "jsonlite", "XML", "assertthat", "lubridate", "scales", "RcppRoll", "zoo", "gridExtra", "futile.logger", "Hmisc", "stringi", "rlang" , "tidyselect" , "readxl", "knitrProgressBar" , "tidyverse" )
...
```{r all_meta }
  ...
  if ( !file.exists( meta_data_file) ){
    ...
    if ( names(md[1]) == "httpStatus" ){ 
        
        url = paste0( baseurl, "api" )
        resources = get( url )[['resources']]
        # View(resources)
        n_resources = nrow( resources )
        
        get_resource_as_list = function( i ){
            
            print( paste( i , resources$plural[i] ) ) 
            
            # named list
            url <- paste0(resources$href[i] , ".json?fields=:all&paging=false" )
        
            schema =  try( get( url ) , silent = TRUE )
            
            # NB: update so that tries up to 3 times...
            
            if ( !is.list( schema ) | schema[3]  %in% "ERROR"){
                 return( data.frame( id = NA ) ) # exit if not a list
            }
        ...
        # get_resource_as_list(i)
    
        md = map(1:n_resources, get_resource_as_list )
        names( md ) = map_chr( 1:n_resources , ~resources$plural[.x] )
        md = md[order(names(md))] #order alphebetically
        # View( md )
    }
...
admins = md$organisationUnits %>% 
 ...
 library( geosphere )
    ...
    # add centroids
    admins$polygons = admins$polygons 
...
- Check if clinic with coordinates is in parent 
...
```{r clinicInParent }
 ...
 facilities_file =  paste0( origin.folder , 
                            dhis_instance,
                            "_updated_facilities.rds" 
                            ) 
 
 if ( file.exists( facilities_file )  ){
     
     c = readRDS( facilities_file ) %>% 
         select( 1:16 )
     ...
     c$has.coordinates = ifelse( is.na( c$lat ) | is.na( c$long ), FALSE, TRUE )
    
     count(c, has.coordinates , level , in.parent , parent.level ) %>% spread( parent.level, n )
...
```{r ImputeLocation }
 ...
 facilities = left_join( c., c.impute ,  by = c( 'id' , 'parent.id' )  ) %>%
     group_by( id ) %>%
     filter( row_number()==n() )  %>% # select last row
     ungroup %>%
...
```{r save_facilities}
 ...
 if ( file.exists( facilities_file ) & params$create_new ){
     ...
     file.rename( facilities_file , paste0( facilities_file , 
                                            format(Sys.time(),
                                                   "%b%d%Y")
     ))
 } 
                                        
 saveRDS( facilities , facilities_file )
...
admins.count$lat = centroid( admins$polygons )[, 2 ]
admins.count$long = centroid( admins$polygons )[, 1 ]
...
- Map of number of org units by admin level
...
```{r de.all }
  ...
  new.dataElements =  setdiff( de.mal$id[ order( de.mal$id )] , 
...
d = dsde %>% 
  ...
  count( d, dataSet, categoryCombos,  periodType, timelyDays  )  %>% kable
