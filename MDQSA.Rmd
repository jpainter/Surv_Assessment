...
knitr::opts_chunk$set( echo = params$echo ,
    ...
    data_directory = params$data_directory  # folder containing country data 
...
```{r parameters2 }
    ...
    dataset.directory = paste0( origin.folder , "datasets/" )
...
```{r dataElement_spreadsheet}
    ...
    de.review.xlsx = paste0( origin.folder , 
                             "dataElements.xlsx" )
...
## Request Data Completeness (form submissions) 


```{r download_dataset_submissions. }
## DataElements : details 

  dataset.submission.file = paste0( dataset.directory , 
                     dhis_instance ,  
...
if ( !exists( "data_details" ) ) data_details = readRDS( paste0(
...
```{r nest.dec }
  ...
  if ( (!exists( "d.nest.dec") | params$create_new ) ) {
    ...
    d.nest = d %>% 
        ...
        group_by( level, level.name, feature, children, orgUnit ,  orgUnit.name , lat, long , 
    ...
    saveRDS( d.nest , 
             paste0( dataset.directory , dhis_instance , "_data_nest.rds") 
    )
