...
```{r}
ous.geos = ous_from_metatdata( md , simplify = TRUE ) %>% filter( !is.na( coordinates ) ) 

## convert geo_json into SF and add other columns (sometimes throws warnings)

ous.sf = geojson_sf( ous.geos$str_js  ) %>% 
    bind_cols(
        ous.geos %>% select( -str_js, -coordinates )
        )
 ...
 if ( !file.exists( ous_file ) ) saveRDS( ous.sf , ous_file ) 
...
```{r c.fix. }
 ...
 for ( i in 1:ncfix ){
        ...
        in.parent.fix[[i]] = map2_lgl( c.to.fix$id[i], 
