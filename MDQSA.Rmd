...
```{r knitr_setup, include = FALSE }
  ...
  knitr::opts_chunk$set( echo = params$echo ,
    ...
    output.directory =  ifelse( is_null( params$output_directory ) , 
...
login to DHIS and fill and copy/replace values into table below

```{r system version}


system_data =  tibble( version = "2.28" ,
                       build_revision = "aff610f" ,
                       build_date = "2018-03-09" , 
                       server_date = "2019-05-06" , 
                       time_since_last_analytics_table_generation = "90 h, 53 m, 56 s" , 
                       last_analytice_table_runtime = "1 h, 38 m, 53 s" )

system_data_file = paste0( dataset.directory , dhis_instance, "_system_data.rds" ) 
saveRDS(system_data, system_data_file )
```
...
### 2. Test facility geocodes to see if they are in their respective


administrative areas (~2-5 min)
...
ous = ous_from_metatdata( .meta = md , simplify = FALSE , SF = TRUE ) 
...
hf = ous.sf %>% filter( feature %in% 'Point' )

admins = ous.sf %>% filter( feature %in% 'Polygon' )
...
keep_the_existing_one = FALSE # so, dont download a new one.  
 ...
 if ( ( file.exists( updated_facilities_file ) & !params$create_new ) | keep_the_existing_one ){
     ...
     # clinics. =  ous.sf$feature %in% 'Point' 
     .clinics = ous %>% filter( clinics. )
     ...
     c = .clinics %>% 
         ...
         ungroup() %>%
         mutate(
             in.parent = map2_lgl( orgUnit, parent_ou , 
                                   ~is.in.parent(.x, .y, 
                                                 .pb = pb, 
                                                 ous = ous.sf ,
             ...
             has.parent = map_lgl( parent_ou , ~.x %in% ous.sf$orgUnit )
         )
 }
...
```{r c.fix. }
 ...
 count( c.to.fix %>% as_tibble() , level ,  in.parent.fix )
 ...
 if ( sum( fixed ) > 0 ){
     ...
     c[ fixed.ous.rows , ] = c.to.fix[ fixed , ]
 }
...
if ( !exists( "facilities" ) ){
    ...
    # TODO:  remake this where c.impute is SF.  return point rather than lat,long -- then update geometry in original SF, c
    
     c.impute = c %>% 
        filter( 
            !has.coordinates | (!in.parent ) , !is.na( parent_ou.name )
            ) %>%
        group_by( orgUnit ) %>%
        filter( row_number() == n() )  %>% # select last row
        select( orgUnit, parent_ou ) %>%
        ungroup
     ...
     for ( i in 1:ncimpute ){
            ...
            impute.coords[[i]] = impute.location(  c.impute$parent_ou[i] ) 
     ...
     c.. = left_join( c, c.impute ,  by = c( 'orgUnit' , 'parent_ou' )  ) %>%
         group_by( orgUnit ) %>%
         filter( row_number()==n() )  %>% # select last row
         ungroup %>%
     ...
     c.. = c %>% filter( !orgUnit %in% c.impute$orgUnit ) %>%
         rbind( c.impute )
...
```{r levels.}
  ...
  levels = ou_levels( md )
    ...
    ## TODO fix error in api_dataset, 'object 'levels.vector' not found'
    levels.vector =  ou_levels( md , name = FALSE ) 
...
```{r search_data_elements. }
    ...
    HIV = quo( grepl( '\\<hiv\\>|vih' , de$name , ignore.case = TRUE ) )
    
    imm = quo( grepl( 'imm' , de$name , ignore.case = TRUE ) )
    
    conf = quo( grepl( 'conf' , de$name , ignore.case = TRUE ) &
            !grepl( 'tb|hiv|vih' , de$name , ignore.case = TRUE )  )
    
    susp = quo( grepl( 'susp' , de$name , ignore.case = TRUE ) &
            !grepl( 'tb|hiv|vih' , de$name , ignore.case = TRUE ) )
    
    presumed = quo( grepl( 'presume|présumé' , de$name , ignore.case = TRUE ) &
            !grepl( 'tb|hiv|vih' , de$name , ignore.case = TRUE ) )
    ...
    patients = quo( 
        grepl( '\\<patient\\>|consult' , de$name , ignore.case = TRUE ) &
            !grepl( '\\<hiv\\>|\\<tb\\>' , de$name , ignore.case = TRUE )
         )
    
    fever = quo( grepl( '\\<fever\\>' , de$name , ignore.case = TRUE )  &
         !grepl( '\\<yellow\\>|\\<typhoid\\|hemor>' , de$name , ignore.case = TRUE )
         )
    ...
    chw = quo( grepl( '\\<chd\\>|\\<ch\\>|\\<chw\\>|\\<communit\\>|\\<iccm\\|\\<icmi\\|\\<imci\\\\<hsa\\||\\<village\\|\\<vhw\\>>' , de$name , ignore.case = TRUE ) )
    
    deaths = quo( 
        grepl( 'death|deaths|dece|deces' , de$name , ignore.case = TRUE ) &
            !grepl( 'tb|hiv|vih' , de$name , ignore.case = TRUE ) 
            )
...
# More complicated search for malaria terms
    ...
    malaria_items = quo( grepl( malaria_search_terms, de$name , 
                      ignore.case = TRUE ) 
                      &
         !grepl( '\\<hiv\\>|vih|\\<tb\\>|MTCT|\\<yellow fever\\>|\\<typhoid fever\\>' , de$name , ignore.case = TRUE )
    )
...
# mean percent of facilities submitting dataset-report
 ...
 percent_dataset_reporting_last_year =  dataset_reporting %>% 
   ...
   dhis2_Join_metadata( "dataSets" , "dataSet") %>%
   select( dataSet.name , forms_expected_lastyear	,
           forms_ontime_lastyear	, form_submission_rate , dataSet  )
...
```{r dataElement_spreadsheet}
    ...
    # previous.include -- if some data already downloaded 
        ...
        if ( file.exists( data.details.file ) ){
            ...
            downloaded = count( data_details, dataElement ) %>%
                pull( dataElement )
        }
   ...
   # if file does not exist, create
    ...
    data_element_info = map( seq_along( searches ) ,
        ...
        , pVal = ifelse( !is.na(expected) , val / expected , NA)
        # 
        # mark to include if reporting?25% or if previously downloaded
        , Include = if( exists( downloaded ) ){
                      ifelse( pVal>.25, "Y" ,  NA ) 
    ...
    select( dataElement.id, dataElement , Include,
            ...
            Percent_Reported , 
            dataSet,
            zeroIsSignificant ,  periodType, Group ,
...
```{r data_elements_include., eval = FALSE  }
    ...
    use_these_sheets = sheets %in% c( "malaria_stock", "stock", "pop", "patients", "attendance",  "malaria_items", "mal" , "deaths", "chw" , "anc", "fever")
...
#             'Attendance', 'ANC', 'Deaths', 'Pop' , 'Stock', 'chw' )

topics  = c( 'mal', 'malaria_items',  'patients', 'fever', 'attendance', 'anc', 'chw', 'deaths', 'pop', 'malaria_stock' )

de.include = 
    map_df(  seq_along( topics ) , 
         ~readxl::read_xlsx( de.review.xlsx ,  sheet = topics[.x] ) %>%
             mutate( Search = topics[.x]   
                     # topic = factor( Search, levels = sheets ,
                                     # labels = topics )
                     ) %>%
           filter( !is.na( Include )  ) 
         # %>%
           # select( Search, dataElement.id , dataElement ) 
) 

de.include = de.include %>%
    # semi_join( de.include , by = c("dataElement.id", "dataElement") ) %>%
    group_by( dataElement.id, dataElement ) %>% 
    mutate( topic = paste0( unique( Search ), collapse = "/")) %>% 
    select( -Search ) %>%
    distinct() %>%  
...
## Unique datasets associated with key variables


```{r unique_datasets_key}
...
key.datasets = 
    ...
    rowwise() %>%
    mutate( topics = paste(unique( unlist( strsplit( topics , "/") )), collapse="/") )
...
```{r data_totals_count, eval=FALSE}
...
## TODO: update dataset translated with new dhis2_join_metadata function

d = dataset.translated( data_details, .ous = ous , .meta = md  ) %>% st_as_sf()
...
<!-- TODO:  output of nest.ts and nest.dec the same?   -->
...
```{r nest.ts }
 ...
 if ( file.exists( nest.ts_file)  ){
  ...
  if ( params$create_new |
    ...
    pb <- progress_estimated( nrow( d.nest.ts ) )
...
de.selected = de.all[ 3 ] #"NMCP OPD Confirmed Malaria Cases Through RDT
...
# choose data element ####
de.coc.all = unique(d.nest.dec$de.coc.name)

de.coc.selected = de.coc.all[3] # "NMCP OPD Suspected Malaria Cases Tested For Malaria Through RDT_<5Yrs"
...
regional.ts = x.ts %>% 
  filter( n >= 24 , !is.na( parent_ou.name ) , level == 4 
          ) %>%
...
tsd.parent = x.dec %>% 
  ...
  mutate( bins = cut( dec , breaks = seq(0, 1, .1 ),  
                      labels = seq(0.05, .95, .1 ) 
                      ) %>% 
...
x.dec %>% 
  ...
  geom_bar(stat = "identity", colour = "black", width = .1 , alpha = .5  ) +
  ggrepel::geom_text_repel( aes( label= orgUnit.name ),
             # position=position_stack(vjust=0.5), 
             colour="black",
             segment.size = 1 ,
             nudge_y = 2, 
...
```{r}
element = 'Confirmed Malaria (only Positive cases)_>5 yrs'

x.dec = d.nest.dec %>% 
  filter( de.coc.name %in% element )  %>%
      # Re-Add geometry for SF
      inner_join( ous %>% select( orgUnit, geometry ) , by = 'orgUnit') %>%
      st_as_sf
...
x.dec.dqa = x.dec %>%
  mutate( 
    ...
    # cap dec at 1.5
    dec = ifelse( is.na( dec ) | dec > 1.5 , 1.5 , dec )
    ) %>%
    st_drop_geometry()

dqa.n = count(  x.dec.dqa , dqa )

colors = c("#006400", "blue", "purple", "#FF4040", "#8B0000")
color.labels =  paste0( dqa.n$dqa, " (n=", dqa.n$n , ")" )

quality.labels = " A: Low error, frequent reporting\nB: high error, infrequent reporting\nC: Low error, infrequent reporting\nD: high error, infrequent reporting"

ggplot( x.dec.dqa , aes( dec, n/4 , color = dqa ) ) +
  geom_point( alpha = .6 ) + 
  scale_color_manual( 
      "Data Quality" , 
      labels = color.labels , breaks = dqa.n$dqa, 
      values =   colors ) + 
  scale_y_continuous( breaks = seq( 0, 12, 2) ) +
  scale_x_continuous( limits =c(0,1.5) ,  breaks = seq( 0, 1.5, .25) ,
                      labels = c( scales::percent(seq( 0, 1.25, .25)) ,
                                  ">150%" ) 
                      ) +
  annotate( 'text', 1.5 , .3 , label = quality.labels , 
            size = rel(2) , hjust = -.4 ) +
  coord_cartesian(clip = 'off') +
  labs( title = "Facility Data Quality", 
        subtitle = element ,
       x = 'Mean percent error' ,
       y = 'Mean number reports per year',
       caption = ""  ) +
  theme_minimal() 
...
region = ous %>% filter(feature %in% 'Polygon', level.name %in% 'SNU1')

reg = rmapshaper::ms_simplify(region, keep = 0.015,
                                          keep_shapes = TRUE)

x.dec.reg = inner_join( reg, x.dec[, c('orgUnit','dec')] , by = c("orgUnit") ) 

tm_shape( x.dec.reg ) +
    tm_polygons(c("orgUnit.name", 'dec')) +
    tm_facets(sync = TRUE, ncol = 2)

hf =  inner_join( 
    ous %>% select( orgUnit, geometry ), 
...
tm =
    ...
    tm_shape(x.dec.reg ) +
    tm_polygons(  'orgUnit.name', alpha = .5 ) +
        tm_shape( hf ) +
        tm_bubbles( col = 'dqa', size = 'total' , 
                    alpha = .9, border.alpha = 0.1 ,
                    scale = .75 , perceptual = TRUE , clustering = TRUE ,
                    id = "orgUnit.name" ,
                    popup.vars = c("facility" = "orgUnit.name", 
                                   "Total reported" = "total" , 
                                   "Number of reports" = "n" , 
                                   "Mean percent error" = "dec") ,
                    popup.format=list( dec=list(fun = scales::percent ) )
        )
tm

## save as stand-alone HTML file ("view" mode)
    map.file = paste0( getwd() , "/", dataset.directory , 
                               dhis_instance, "_hf_.html" )

    tmap_save( tm, filename = map.file )
...
i = 9  ; de.coc.all[9] # "NMCP OPD Confirmed Malaria Cases Through RDT_<5Yrs"
