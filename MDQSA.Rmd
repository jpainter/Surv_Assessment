...
clinics. = clinics %>%
    ...
    left_join( admins %>% 
                    select( id, name ) %>% 
                    rename( admin.name = name, admin.id = id ) , 
                by = c( 'parent.id' = 'admin.id' ) 
                ) %>%
...
```{r facilities_reporting_last_year, eval = FALSE }
...
malaria.groups = setdiff( names(searches) , c("TB", "imm", "HIV", "opd", "idp", "susp", "conf", "stock", "deaths") )
...
```{r data_elements_include., eval = FALSE  }
    ...
    use_these_sheets = sheets %in% c( "malaria_stock", "stock", "pop", "attendance", "opd" , "ipd" , "malaria_items", "deaths" )
