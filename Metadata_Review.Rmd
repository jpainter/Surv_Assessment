...
# **`r toupper( params$dhis_instance )` Metadata**
...
# list of required packages
    package.list = c("rlist", "listviewer", "DT", "knitr", "kableExtra", "xts", "leaflet", "RColorBrewer", "DT", "dygraphs", "httr", "jsonlite", "XML", "assertthat", "lubridate", "scales", "RcppRoll", "zoo", "gridExtra", "futile.logger", "Hmisc", "stringi", "rlang" , "tidyverse" )
...
String search for data elements.  Note that when \\<*words*<\\ are surrounded by '\\<', the search is looking for the whole word (with leading and trailing space). 
...
# Organise all the searches as a list
    searches = list( mal, malaria_items, TB, HIV, imm, conf, susp, opd, ipd, patients, attendance, anc, pop )
    
    names.searches  = c( 'mal', 'malaria_items', 'TB', 'HIV', 'imm', 'conf', 'susp', 'opd', 'ipd', 'patients', 'attendance', 'anc', 'pop' )
    ...
    search = 
        map_df( seq_along(searches), 
                ~data_frame(
                    ...
                    search_text =  paste( f_rhs( searches[[.x]] ), 
                                         collapse = " " ) ,
                    Count = sum( eval_tidy( searches[[.x]] ) )
    ))
    
    kable( search , row.names = TRUE)
...
TODO: Inspect organization of data Element groups?  Which var are assigned to groups?  Are elements assigned to more than one group? 

- Look at other dataElements in the group to see if missing something.
...
```{r select_data_elements}
    ...
    # review names and select most appropriate
    # de.names = unique( de$name[ search$index[[1]]| #malaria
    #                                 search$index[[10]]| #ipd
    #                                 search$index[[8]]| #attendance
    #                                 search$index[[11]] ] ) #population

    de.names.review = map_df( c(mal, pop) ,
                              ~ de.[ eval_tidy( .x ) , c('name', 'Group') ] ) %>%
            rename( dataElement = name ) %>%
            mutate(  Include = NA ) %>%
            select( Include, dataElement ,  Group )

    # de.names_review = data_frame( 
    #     Include = rep( NA, nrow( de.names ) ) ,
    #     dataElement = de.names )
    ...
    if ( !file.exists( de.review.xlsx ) ) openxlsx::write.xlsx( de.names.review ,
                                                                de.review.xlsx )
