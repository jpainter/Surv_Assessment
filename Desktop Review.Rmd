...
```{r ous_by_level}
 ...
 table( year( ymd_hms( ous$openingDate )) ) # but some are recent
  ...
  ous.n$period = fast_strptime( as.character(  ous.n$period ) , "%Y%m") 
  
  ggplot( ous.n,  aes(x = period, y = n )) + 
      geom_point() +
...
```{r datasets }
  ...
  mal.datasets = md$dataSets$name[ c( 19, 20, 32, 37, 40 , 44, 45, 64:65, 67, 116, 117 )]
...
sus = grepl( 'susp' , de$name , ignore.case = TRUE )
