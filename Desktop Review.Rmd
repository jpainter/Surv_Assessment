...
```{r fetch_dataset_completeness }
  ...
  data.file = paste0( folder , 
    ...
    # datasets
    datasets =  unique( mal.datasets$dataset.id )
    ...
    dataset.data[[i]] = data.table::rbindlist( dataset.level )
      ...
      # combine period data
      dataset = data.table::rbindlist( dataset.data )
...
```{r}
    ...
    s = dataset %>%
        separate( dataElement, c('dataElement.id', 'Category' ), sep = '\\.' ) %>%
        left_join(  select( ous , id, level) , by = c('orgUnit' = 'id') ) %>%
        group_by( level, period , dataElement.id , Category ) %>%
        summarise( value = sum( as.integer( value) ) ) %>%
        ungroup() %>% 
        spread( Category, value ) %>%
        mutate( submitted = ACTUAL_REPORTS / EXPECTED_REPORTS ) %>%
         # level names
         left_join( 
    ...
    labs( title = 'Dataset submission by level', 
