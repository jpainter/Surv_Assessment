...
```{r select_data_elements}
    ...
    search_words = c('malaria','conf','susp','\\<patient\\>', '\\<attendance\\>', 'pop')
    search = 
    ...
    de.names = unique( de$name[ search$index[[1]]| #malaria
                                    search$index[[5]]| #attendance
                                    search$index[[6]] ] ) #population
...
```{r fetch_data_values_details_populations }

  # population variables
  de.names = unique( de$name[ search$index[[6]] ] )

  data.file = paste0( origin.folder , 
                     params$origin_login_file, 
                     "_dataset_details_populations.rds" )
    ...
    data[[i]] = data.table::rbindlist( data.level )
      ...
      # combine period data
      data_details_populations = data.table::rbindlist( data )
      
      print( paste( "TOTAL:", scales::comma( nrow( data_details ) ) , "records"  ) )
    
      # glimpse(data)
      
      saveRDS( data_details_populations, data.file )
}

glimpse( data_details_populations ) 
