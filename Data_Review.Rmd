...
# list of required packages
    package.list = c("rlist", "knitr", "kableExtra", "xts", "leaflet", "RColorBrewer", "DT", "dygraphs", "httr", "jsonlite", "XML", "assertthat", "lubridate", "scales", "RcppRoll", "zoo", "gridExtra", "futile.logger", "zoo" , "broom" , "tidyverse" )
...
- Lets select some data elements using key word (or parital word) searches
...
```{r select_data_elements}
    ...
    patients = grepl( '\\<patient\\>' , de$name , ignore.case = TRUE ) &
         !grepl( '\\<hiv\\>' , de$name , ignore.case = TRUE ) &
         !grepl( '\\<tb\\>' , de$name , ignore.case = TRUE )
    ...
    kable( search %>% select( search_string, count ), row.names = TRUE)
    ...
    section.size = 3
    de.names.sections = ceiling( length( de.names ) / section.size )
    ...
    for ( section in 1:de.names.sections){
                ...
                if (stop > length(de.names) ) stop = length(de.names)
                
                de.name.part[[section]] = de.names[ start:stop ] 
        }
...
```{r fetch_data_values_total , eval= FALSE }
  ...
  data.file = paste0( origin.folder , 
    ...
    data[[i]] = data.table::rbindlist( data.level , fill = TRUE )
      ...
      saveRDS( fetch$lastUpdated[1] , paste0(data.file, "_date") )
}
...
```{r fetch_data_values_details }
  ...
  data.file = paste0( origin.folder , 
    ...
    # import month by month , level, and data element
      for ( i in seq_along(periods.vector) ){
          ...
          print( levels.vector[level] )
            ...
            for ( de.section in 1:de.names.sections ){
              ...
              # data elements to get:
                ...
                print( paste( 'part', de.section , "/" , de.names.sections) )
        ...
        url <- paste0( baseurl, "api/analytics/dataValueSet.json?" ,
            ...
            print( paste( scales::comma( nrow( data.level[[level]] ) ) , "records"  ) )
        }
    ...
    data[[i]] = data.table::rbindlist( data.level, fill = TRUE )
      ...
      saveRDS( max( data_details$lastUpdated, na.rm =  TRUE ) , paste0(data.file, "_date") )
}

    glimpse( data_details ) 
    
    data_date = readRDS(  paste0(data.file, "_date") )
```

The data was downloaded on `r data_date`
